 ...
 * @since 1.0.0
 */
class Themename_Widget_Base extends WP_Widget {
	...
	private function sanitize( $key, $field, $value ) {
		...
		switch ( $field_type ) {
			...
			case 'select':
			case 'radio':
				$inputColor = esc_attr( $value );
				$choices = $field['options'];
				$output = array_key_exists( $inputColor, $choices ) ? $inputColor : $field['default'];
				break;
			case 'checkbox':
				$output = ! empty( $value );
				break;
		...
		switch ( $field_type ) {
			...
			case 'image':
				...
				<div>
					...
					<!-- <br /> -->
					<input type="button" class="select-img button button-primary" value="<?php esc_attr_e( 'Upload', 'themename' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'themename' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'themename' ); ?>" />
					<?php
					$image_status = false;
					if ( ! empty( $value ) ) {
						$image_status = true;
					...
					?>
					<input type="button" value="<?php echo esc_attr_x( 'X', 'Remove', 'themename' ); ?>" class="button button-secondary btn-image-remove" style="<?php echo esc_attr( $remove_button_style ); ?>" />

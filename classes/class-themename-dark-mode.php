...
class Theme_Name_Dark_Mode {
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && Theme_Name_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
			// Add Dark Mode variable overrides.
			wp_add_inline_style(
				'themename-custom-color-overrides',
				'.is-dark-theme.is-dark-theme .editor-styles-wrapper { --Background: var(--global--color-dark-gray); --primary: var(--global--color-light-gray); --secondary: var(--global--color-light-gray); --button--color-text: var(--Background); --ButtonText-hover: var(--secondary); --button--color-text-active: var(--secondary); --ButtonBackground: var(--secondary); --ButtonBackground-active: var(--Background); --global--color-border: #9ea1a7; --table--stripes-border-color: rgba(240, 240, 240, 0.15); --table--stripes-background-color: rgba(240, 240, 240, 0.15); }'
			);
		}
		wp_enqueue_script(
			'themename-dark-mode-support-toggle',
			get_template_directory_uri() . '/js/dark-mode-toggler.js',
			array(),
			'1.0.0',
			true
		);
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/js/editor-dark-mode-support.js',
	...
	public function enqueue_scripts() {
		...
		$url = get_template_directory_uri() . '/assets/themename/css/style-dark-mode.css';
		if ( is_rtl() ) {
			$url = get_template_directory_uri() . '/assets/themename/css/style-dark-mode-rtl.css';
		}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/js/customize.js',
	...
	public function customizer_controls( $wp_customize ) {
		...
		$wp_customize->add_control(
			new Theme_Name_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => static function() {
						return 127 >= Theme_Name_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= Theme_Name_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= Theme_Name_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function the_script() {
		echo '<script>';
		include get_template_directory() . '/js/dark-mode-toggler.js'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
		echo '</script>';
	}

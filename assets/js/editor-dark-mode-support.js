/* global themenameIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'themename-supports-dark-theme' ) ) {
	themenameDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function themenameDarkModeEditorInit( attempt ) {
	...
	if ( themenameIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						themenameDarkModeEditorInit( attempt + 1 );
					},
